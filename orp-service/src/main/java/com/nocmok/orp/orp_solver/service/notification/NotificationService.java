...
public class NotificationService {
    ...
    @Autowired
    public NotificationService(AssignRequestNotificationMapper assignRequestNotificationMapper,
    ...
    public void sendNotification(RerouteNotification rerouteNotification) {
        log.info("------> SEND REROUTE NOTIFICATION <------");
        var record = OrpOutputOutboxRecord.builder()
                .messageId(null)
                .partitionKey(rerouteNotification.getSessionId())
                .payload(rerouteNotification)
                .createdAt(Instant.now())
                .messageKind(com.nocmok.orp.kafka.orp_output.RerouteNotification.class.getName())
                .sentAt(null)
                .build();
        log.info("notification scheduled {}", record.getPayload());
        orpOutputOutboxStorage.insertOneRecord(record);
